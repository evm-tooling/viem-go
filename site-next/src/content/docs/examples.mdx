...
Interacting with smart contracts is the core of most Ethereum applications. These examples cover reading on-chain state through view functions and encoding calldata for contract writes — the two operations you'll use most often.
...
Manually encode calldata from an ABI and arguments. This is useful when you need raw calldata for low-level `eth_call` or `eth_sendTransaction` without the higher-level `ReadContract`/`WriteContract` helpers.
...
Accounts are how you sign transactions and messages. viem-go supports creating accounts from raw private keys or BIP-39 mnemonics, with the same API shape as viem's `accounts` module.
...
Derive an account from a BIP-39 mnemonic phrase. An optional derivation path lets you generate multiple accounts from the same seed.
...
Helpers for the low-level operations that come up constantly in Ethereum development — unit conversions between human-readable values and raw integers, hashing, and cryptographic signature recovery.
...
Keccak256 hashing, EIP-191 message signing, and `ecrecover`-based address recovery. These are the building blocks for signature verification, permit flows, and off-chain authentication.
...
Production-grade patterns that go beyond single RPC calls. The snippets below are cherry-picked from [`viem-go-extractor-demo`](https://github.com/ChefBingbong/viem-go-extractor-demo/tree/main) — a real UniswapV2 pool extractor that heavily exercises **multicall batching**, **concurrent fan-out**, **reorg-aware log watching**, and **on-disk caching**. These show how viem-go's primitives compose into non-trivial data pipelines.
